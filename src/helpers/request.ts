...
const curlOnce = (
  ...
  new Promise((resolve) => {
    ...
    curlHandle.enable(CurlFeature.Raw);
    curlHandle.setOpt("URL", url);
    if (site.headers)
      curlHandle.setOpt(Curl.option.HTTPHEADER, site.headers.map(replaceEnvironmentVariables));
    if (site.body) curlHandle.setOpt("POSTFIELDS", replaceEnvironmentVariables(site.body));
...
export const curl = async (
  ...
  let lastResult: { httpCode: number; totalTime: number; data: string; error?: Error } = {
    httpCode: 0,
    totalTime: 0,
    data: "",
  };
  ...
  console.log(
    `All ${maxRetries} curl attempts failed for ${replaceEnvironmentVariables(site.url)}, returning HTTP 0`
  );
